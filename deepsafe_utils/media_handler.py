...
class MediaHandler:
    ...
    def find_media_files(
        self, input_dir: str, media_type: str
    ) -> List[Tuple[str, str]]:
        media_items = []
        ...
        if not input_dir or not os.path.isdir(input_dir):
            console.print(
                f"[bold red]Input directory '{input_dir}' does not exist or is invalid.[/bold red]"
            )
            return []

        if not supported_extensions:
            console.print(
                f"[bold red]No supported extensions defined for media type '{media_type}' in config.[/bold red]"
            )
            return []

        fake_dir_options = [
            os.path.join(input_dir, "Fake"),
            os.path.join(input_dir, "fake"),
        ]
        real_dir_options = [
            os.path.join(input_dir, "Real"),
            os.path.join(input_dir, "real"),
        ]
        ...
        if fake_dir and real_dir:
            console.print(
                f"Searching for {media_type} files in standard Real/Fake subdirectories..."
            )
            for dir_path_actual, label in [(fake_dir, "Fake"), (real_dir, "Real")]:
                for ext_pattern in supported_extensions:
                    for media_file_path in glob.glob(
                        os.path.join(dir_path_actual, "**", ext_pattern), recursive=True
                    ):
            ...
            console.print(
                f"[yellow]Warning: Real/Fake subdirectories not fully found. Searching in '{input_dir}' directly and inferring labels for {media_type}s.[/yellow]"
            )
            found_in_root = []
            for ext_pattern in supported_extensions:
            ...
            if found_in_root:
                for media_file_path in found_in_root:
                    if media_file_path in found_paths:
                        continue  # Already processed if, somehow, structure was partially there
                ...
                console.print(
                    f"[bold red]Error: No Real/Fake subdirectories found, and no {media_type} files found directly in {input_dir} matching extensions: {supported_extensions}[/bold red]"
                )

        fake_count = sum(1 for _, label in media_items if label == "Fake")
        real_count = sum(1 for _, label in media_items if label == "Real")
        console.print(
            f"Found {len(media_items)} {media_type}(s): {fake_count} Fake, {real_count} Real."
        )

        if not media_items:
            console.print(
                f"[bold red]No {media_type} files found in '{input_dir}' based on configuration.[/bold red]"
            )
        return media_items
    ...
    def encode_media_to_base64(self, media_path: str) -> Optional[str]:
        try:
            ...
            console.print(
                f"[bold red]Error encoding media '{media_path}': {e}[/bold red]"
            )
            return None
    ...
    def validate_media_file(self, media_path: str, media_type: str) -> bool:
        ...
        if media_type == "image":
            try:
                ...
                console.print(
                    f"[red]Invalid or corrupt image file {media_path}: {e}[/red]"
                )
                return False
