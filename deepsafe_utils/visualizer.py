...
class Visualizer:
    def __init__(self, config_manager, output_dir_for_run: str):
        ...
        self.output_dir_for_run = (
            output_dir_for_run  # Base for plots for this specific run
        )
    ...
    def _group_results(
        ...
        for res_item in results_list:
            ...
            if (
                not model_name
                or "error" in res_item
                or res_item.get("ground_truth") == "Unknown"
                or res_item.get("prediction") is None
                or res_item.get("probability") is None
            ):
                continue
    ...
    def plot_confusion_matrices(self, all_results: List[Dict[str, Any]]):
        ...
        if not grouped_results:
            console.print(
                "[yellow]No valid results to plot confusion matrices.[/yellow]"
            )
            return
        ...
        for model_name, model_specific_results in grouped_results.items():
            y_true = [
                1 if r["ground_truth"] == "Fake" else 0 for r in model_specific_results
            ]
            y_pred_class = [int(r["prediction"]) for r in model_specific_results]

            if not y_true or (
                len(set(y_true)) < 1 and len(y_true) > 0
            ):  # Needs at least one class for CM, if y_true is empty, skip
                console.print(
                    f"[yellow]Skipping CM for {model_name}: Insufficient or single-class data.[/yellow]"
                )
                continue
            ...
            try:
                ...
                plot_path = os.path.join(
                    cm_dir, f"cm_{model_name.replace('/', '_').replace(' ', '_')}.png"
                )
        ...
        if os.listdir(cm_dir):
            console.print(f"Confusion matrices saved to [green]{cm_dir}[/green]")
    ...
    def plot_roc_curves(self, all_results: List[Dict[str, Any]]):
        ...
        for model_name, model_specific_results in grouped_results.items():
            ...
            try:
                ...
                if pd.notna(roc_auc_val):  # Only plot if AUC is valid
                    plt.plot(
                        fpr, tpr, lw=2, label=f"{model_name} (AUC = {roc_auc_val:.3f})"
                    )
                    plot_count += 1
            except Exception as e:
                console.print(
                    f"[red]Error plotting ROC curve for {model_name}: {e}[/red]"
                )
        ...
        if plot_count > 0:
            ...
            plt.legend(
                loc="lower right", fontsize="small", frameon=True, shadow=False
            )  # Adjusted legend
            plt.grid(alpha=0.35)
            plt.tight_layout()
            ...
            plt.savefig(plot_path, dpi=150)
            console.print(
                f"Combined ROC curves plot saved to [green]{plot_path}[/green]"
            )
        else:
            console.print(
                "[yellow]No valid ROC curves were plotted (e.g., due to single class in y_true or constant y_pred_proba).[/yellow]"
            )
        plt.close()
    ...
    def plot_probability_distributions(self, all_results: List[Dict[str, Any]]):
        ...
        if not grouped_results:
            console.print(
                "[yellow]No valid results to plot probability distributions.[/yellow]"
            )
            return
        ...
        for model_name, model_specific_results in grouped_results.items():
            ...
            if (
                "probability" not in df_model.columns
                or "ground_truth" not in df_model.columns
            ):
                continue

            df_model["probability"] = pd.to_numeric(
                df_model["probability"], errors="coerce"
            ).dropna()
            if df_model.empty:
                continue
            ...
            if real_probs.empty and fake_probs.empty:
                continue
            ...
            plt.xlim(-0.05, 1.05)
            plot_path = os.path.join(
                dist_dir, f"dist_{model_name.replace('/', '_').replace(' ', '_')}.png"
            )
            plt.savefig(plot_path, dpi=150)
            plt.close()
        if os.listdir(dist_dir):
            console.print(
                f"Probability distributions saved to [green]{dist_dir}[/green]"
            )
    ...
    def plot_benchmark_summary(self, benchmark_data: Dict[str, Any]):
        ...
        if (
            api_bench_data.get("mean_total_request_time") is not None
            and api_bench_data.get("count", 0) > 0
        ):
        ...
        for model_name, data in sorted(models_bench_data.items()):
            if (
                data.get("mean_total_request_time") is not None
        ...
        for bar in bars:
            ...
            plt.text(
                ...
                yval + 0.01 * max(mean_times, default=1),
                f"{yval:.3f}s",
                ha="center",
                va="bottom",
                fontsize=8,
        ...
        plt.close()
        console.print(
            f"Benchmark mean times plot saved to [green]{os.path.join(output_plot_dir, 'benchmark_mean_times.png')}[/green]"
        )
        ...
        if any(all_times_raw):
            ...
            plt.tight_layout()
            plt.savefig(
                os.path.join(output_plot_dir, "benchmark_distributions.png"), dpi=150
            )
            plt.close()
            console.print(
                f"Benchmark distributions plot saved to [green]{os.path.join(output_plot_dir, 'benchmark_distributions.png')}[/green]"
            )
        else:
            console.print(
                "[yellow]No raw time data available for distribution plot.[/yellow]"
            )
