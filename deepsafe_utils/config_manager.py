...
class ConfigManager:
    ...
    def __new__(cls, config_path: Optional[str] = None):
        if cls._instance is None:
            ...
            )  # Assumes utils is one level down
            cls._instance.config_path = config_path or os.path.join(
    ...
    def load_config(self) -> Dict[str, Any]:
        try:
            ...
            console.print(
                f"[bold red]ERROR: Configuration file not found at {self.config_path}. Please create it based on the template.[/bold red]"
            )
            ...
            console.print(
                f"[bold red]ERROR: Invalid JSON in configuration file: {self.config_path}.[/bold red]"
            )
            return {
                "api_url": "http://localhost:8000",
                "media_types": {
