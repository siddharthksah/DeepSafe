...
def upload_eval_output(output_probs, output_times, num_frames, job_name, total_time):
    ...
    _upload_output_to_s3(upload_data, filename,
                         WORKSPACE_BUCKET, UPLOAD_PREFIX)
...
def get_frames_iter():
    ...
    for video_id in video_list:
        ...
        st = time.time()
        try:
            _download_s3_video(WORKSPACE_BUCKET, os.path.join(
                VIDEO_PREFIX, video_id), video_id)
        except:
            logging.info("Failed to download video: {}".format(
                os.path.join(VIDEO_PREFIX, video_id)))
            raise
        # local path of the video named video_id
        video_local_path = os.path.join(TMP_PATH, video_id)
        frames = extract_frames(video_local_path)
        elapsed = time.time() - st
        logging.info(
            "video downloading & frames extracting time: {}".format(elapsed))
        yield video_id, frames
        try:
            os.remove(video_local_path)  # remove the video named video_id
        except:
            logging.info(
                "Failed to delete this video, error: {}".format(sys.exc_info()[0]))


def get_local_frames_iter(video_path, max_number=None):
    """
    This function returns a iterator of frames of test videos.
    ...
    try:
        ...
        logging.info("Failed to read image: {}".format(video_path))
        raise
    return frames
...
def verify_local_output(output_probs, output_times, num_frames):
    ...
    for k in gts:
        ...
        assert k in output_probs and k in output_times, ValueError(
            "The detector doesn't work on video {}".format(k))
